...
export async function showRecordingNotification(): Promise<void> {
  try {
    ...
    if (showNotification) {
      ...
      const toastId = toast.info('🔴 Đã bắt đầu ghi âm', {
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-gray-900">
              Hãy thông báo cho mọi người tham dự biết cuộc họp đang được ghi âm.
            </p>
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-100 p-2 rounded transition-colors">
              ...
              />
              <span className="select-none text-gray-700">Không hiển thị lại</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
