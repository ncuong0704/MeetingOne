...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|MeetingOne-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/MeetingOne-backend/0.0.4/backend/meeting_minutes.db"
    ...
    local common_paths=(
        "/opt/homebrew/Cellar/MeetingOne-backend/*/backend/meeting_minutes.db"
        "$HOME/.MeetingOne/meeting_minutes.db"
        "$HOME/Documents/MeetingOne/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "MeetingOne-backend:"; then
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "MeetingOne-backend"; then
        app_running=true
        local app_port=$(docker port MeetingOne-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
...
open_shell() {
    ...
    case "$service" in
        "whisper")
            ...
            ;;
        "app"|"backend")
            container_name="MeetingOne-backend"
            ;;
        *)
