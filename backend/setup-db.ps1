...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.MeetingOne\meeting_minutes.db",
        "$env:USERPROFILE\Documents\MeetingOne\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/MeetingOne-backend/*/backend/meeting_minutes.db"
    }
