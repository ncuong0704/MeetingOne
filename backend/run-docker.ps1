...
$AppContainerName = "MeetingOne-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "MeetingOne-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server MeetingOne-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server MeetingOne-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "MeetingOne-backend"
        "backend" = "MeetingOne-backend"
        "meeting" = "MeetingOne-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "MeetingOne-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, MeetingOne-backend"
        ...
        $logArgs += @("whisper-server", "MeetingOne-backend")
    }
