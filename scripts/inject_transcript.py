...
Injects CSV-based transcript data into the MeetingOne SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "MeetingOne"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "MeetingOne"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "MeetingOne"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "MeetingOne"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into MeetingOne database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure MeetingOne has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the MeetingOne sidebar.")
